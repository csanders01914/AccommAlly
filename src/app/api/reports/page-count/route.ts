...
export async function GET(_request: NextRequest) {
 ...
 const stripePaymentIntentIdEnc = encrypt('free_export_' + Date.now() + Math.random().toString());
 const record = await prisma.reportExportPayment.create({
 data: {
 ...
 pageCount,
 amountCents: 0,
 amountDisplay: "Free (Included in Plan)",
 isFree: true,
 exportToken
 });
 }
