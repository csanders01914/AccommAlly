...
export default function TemplatesPage() {
 ...
 <div>
 <Link href={`/super-admin/tenants/${tenantId}`} className="text-sm text-slate-500 hover:text-slate-700 mb-1 block">
 ← Back to Tenant
 </Link>
 <h1 className="text-2xl font-bold text-slate-900">Letter Templates</h1>
 <p className="text-slate-500 text-sm mt-1">DOCX templates with variable substitution for coordinators.</p>
 </div>
 <button
 onClick={openCreate}
 ...
 {/* Template list */}
 {templates.length === 0 ? (
 <div className="text-center py-12 text-slate-500">No templates yet. Upload a .docx file to get started.</div>
 ) : (
 ...
 <tbody>
 {templates.map(t => (
 <tr key={t.id} className="border-b border-slate-200 hover:bg-slate-200">
 <td className="p-4">
 <p className="font-medium text-slate-900">{t.name}</p>
 {t.description && <p className="text-slate-500 text-xs mt-0.5">{t.description}</p>}
 </td>
 <td className="p-4 text-slate-600">{t.variableMappings?.length ?? 0} custom</td>
 ...
 <button onClick={() => openEdit(t)} className="p-1.5 text-slate-500 hover:text-slate-900" title="Edit">
 <Edit2 className="w-4 h-4" />
 </button>
 <button onClick={() => handleDelete(t.id, t.name)} className="p-1.5 text-slate-500 hover:text-red-400" title="Delete">
 ...
 className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-900 text-sm"
 ...
 className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-900 text-sm resize-none h-20"
 ...
 className="w-full text-sm text-slate-600 file:mr-3 file:py-1.5 file:px-3 file:rounded file:border-0 file:bg-indigo-700 file:text-slate-900 file:text-sm"
 />
 </div>
 ...
 <label className="text-sm font-medium text-slate-600">Variable Mappings</label>
 <button
 type="button"
 ...
 className="flex-1 px-3 py-1.5 bg-slate-700 border border-slate-600 rounded text-slate-900 text-sm font-mono"
 />
 <select
 value={m.field}
 onChange={e => updateMapping(i, 'field', e.target.value as TemplateField)}
 className="flex-1 px-3 py-1.5 bg-slate-700 border border-slate-600 rounded text-slate-900 text-sm"
 >
 {TEMPLATE_FIELDS.map(f => (
 <option key={f} value={f}>{TEMPLATE_FIELD_LABELS[f]}</option>
 ))}
 </select>
 <button onClick={() => removeMapping(i)} className="text-slate-500 hover:text-red-400">
 <X className="w-4 h-4" />
 </button>
 </div>
 ))}
 ...
 className="w-full flex items-center justify-between p-3 text-sm text-slate-500 hover:text-slate-700"
 >
 ...
 <button onClick={() => setShowForm(false)} className="px-4 py-2 text-slate-500 hover:text-slate-900 text-sm">
 Cancel
 </button>
 <button
 onClick={handleSave}
