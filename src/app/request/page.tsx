...
export default function AccommodationRequestPage() {
 ...
 <div className="absolute top-1/2 left-0 w-full h-0.5 bg-[#E5E2DB] -z-10" />
 {STEPS.map((s) => (
 <div
 key={s.number}
...
function Input({ label, error, icon, ...props }: InputProps) {
 ...
 {error && <p className="text-red-500 text-sm mt-1 flex items-center gap-1" role="alert"><AlertCircle className="w-3 h-3" /> {error}</p>}
 </div>
 );
}
