...
export default function LinkClaimsModal({ isOpen, onClose, caseId, onSuccess, currentFamily }: LinkClaimsModalProps) {
 ...
 <div className="modal-container w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
 ...
 <div>
 <label className={labelCls}>Family Name <span className="normal-case font-normal text-text-muted">(optional)</span></label>
 ...
 <p className="text-sm text-text-muted">No other claims found for this claimant.</p>
 </div>
 ) : (
 ...
 className={cn('flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all', isSelected ? 'bg-primary-500/8 border-primary-500/30' : 'bg-surface border-border hover:border-primary-500/30 hover:bg-background')}>
 <div className={cn('mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-colors flex-shrink-0', isSelected ? 'bg-primary-500 border-primary-500 text-white' : 'border-border-strong bg-surface')}>
 {isSelected && <CheckCircle className="w-3.5 h-3.5" />}
 </div>
 <div className="flex-1">
 <div className="flex items-center justify-between">
 <span className="font-medium text-sm text-text-primary">#{suggestion.caseNumber}</span>
 <span className="text-xs px-2 py-0.5 bg-surface-raised rounded-full text-text-secondary">{suggestion.status}</span>
 </div>
 <p className="text-sm text-text-secondary mt-1">{suggestion.title}</p>
 {suggestion.descriptionSnippet && <p className="text-xs text-text-muted mt-0.5 line-clamp-1">{suggestion.descriptionSnippet}</p>}
 ...
 <button onClick={handleSubmit} disabled={isSubmitting} className="flex items-center gap-2 px-5 py-2 bg-primary-500 hover:bg-primary-600 text-white rounded-lg text-sm font-semibold transition-colors disabled:opacity-70">
