...
export function AccommodationModal({ isOpen, onClose, onSave, initialData, caseId }: AccommodationModalProps) {
 ...
 <div className="flex items-center justify-between">
 <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-text-muted">Decision Logic</p>
 <label className="flex items-center gap-2 text-sm text-text-secondary cursor-pointer">
 ...
 <span className="text-sm text-text-primary font-medium">{q.label}</span>
 <div className="flex bg-surface border border-border rounded-lg overflow-hidden shrink-0">
 <button type="button" onClick={() => updateLogic(q.key as keyof DecisionLogicAnswers, true)} className={`px-3 py-1 text-xs font-semibold flex items-center gap-1 transition-colors ${val === true ? 'bg-green-100 text-green-700' : 'text-text-muted hover:bg-surface-raised'}`}>
 <Check className="w-3 h-3" /> Yes
 </button>
 <div className="w-px bg-border"></div>
 <button type="button" onClick={() => updateLogic(q.key as keyof DecisionLogicAnswers, false)} className={`px-3 py-1 text-xs font-semibold flex items-center gap-1 transition-colors ${val === false ? 'bg-red-100 text-red-700' : 'text-text-muted hover:bg-surface-raised'}`}>
 ...
 <Info className={`w-5 h-5 shrink-0 ${suggestion === 'APPROVE' ? 'text-green-500' : suggestion === 'REQUEST_INFO' ? 'text-yellow-500' : suggestion === 'REVIEW_FOR_DENIAL' ? 'text-red-500' : 'text-text-muted'}`} />
 <span>
 ...
 <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-text-muted">Request Status</p>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
 <div>
 ...
 <button type="submit" disabled={isLoading} className="px-5 py-2 text-sm font-semibold text-white bg-primary-500 hover:bg-primary-600 rounded-lg disabled:opacity-50 transition-colors">
