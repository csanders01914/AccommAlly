...
function StatChip({ label, value, tone = 'neutral' }: {
    ...
    return (
        <div className="inline-flex items-baseline gap-2 px-3 py-1.5 rounded-full border border-white/[0.12] bg-[#ffffff]/[0.04]">
...
function StatusDot({ status }: { status: string }) {
    const colors: Record<string, string> = {
        ...
        CANCELLED: '#8C8880', OPEN: '#0D9488', CLOSED: '#8C8880',
    };
    return (
...
function CaseChip({ caseNumber, danger = false }: { caseNumber: string; danger?: boolean }) {
    return (
        <code className={`font-mono text-[11px] px-1.5 py-0.5 rounded ${danger ? 'bg-red-50 text-red-800' : 'bg-teal-50 text-teal-800'}`}>
            {caseNumber}
        </code>
    );
}
...
function Card({ title, icon: Icon, action, children }: {
    ...
    return (
        <section className="bg-[#ffffff] border border-[#E5E2DB] rounded-xl shadow-[0_1px_2px_rgba(28,26,23,0.04)] overflow-hidden flex flex-col">
            <header className="flex items-center justify-between px-5 py-3 border-b border-[#F3F1EC]">
                <div className="flex items-center gap-2.5">
                    <Icon className="w-3.5 h-3.5 text-[#0D9488]" />
                    <h3 className="text-sm font-semibold text-[#1C1A17]">{title}</h3>
                </div>
                {action}
            </header>
            {children}
...
function TasksCard({ tasks }: { tasks: DashTask[] }) {
    ...
    return (
        <Card
            ...
            icon={ListTodo}
            action={
                <Link href="/dashboard/tasks" className="text-xs font-medium text-[#0D9488] hover:text-[#0F766E]">
                    View all →
                </Link>
            }
        >
            {/* Table header */}
            <div className="grid grid-cols-[140px_180px_1fr_16px] gap-4 px-5 py-2.5 bg-[#F8F7F5] border-b border-[#E5E2DB] text-[10px] font-semibold uppercase tracking-[0.12em] text-[#8C8880]">
                <div>Due</div>
                <div>Claim #</div>
                <div>Description</div>
                <div />
            ...
            {shown.length === 0 ? (
                <p className="px-5 py-8 text-sm text-[#8C8880] text-center">No pending tasks.</p>
            ) : (
                shown.map(t => {
                    ...
                    return (
                        <Link
                            ...
                            className={`grid grid-cols-[140px_180px_1fr_16px] gap-4 px-5 py-3.5 border-b border-[#F3F1EC] items-center transition-colors last:border-b-0 ${over ? 'bg-[rgba(254,242,242,0.45)] hover:bg-[rgba(254,242,242,0.7)]' : 'bg-[#ffffff] hover:bg-[#F8F7F5]'}`}
                        >
                            <div className="flex items-center gap-1.5">
                                {over
                                    ? <AlertTriangle className="w-3.5 h-3.5 text-red-500 flex-shrink-0" />
                                    : <Clock className="w-3.5 h-3.5 text-[#8C8880] flex-shrink-0" />
                                }
                                <span className={`text-[13px] font-medium tabular-nums ${over ? 'text-red-600' : 'text-[#1C1A17]'}`}>
                                    {t.dueDate ? format(new Date(t.dueDate), 'MMM d') : '—'}
                                </span>
                                {today && !over && (
                                    <span className="text-[10px] font-semibold tracking-wide text-[#0D9488]">TODAY</span>
                                )}
                            </div>
                            <CaseChip caseNumber={t.claimNumber || t.case?.caseNumber || '—'} danger={over} />
                            <div className="min-w-0">
                                <p className="text-[13px] text-[#1C1A17] truncate leading-snug">{t.title}</p>
                                <div className="flex gap-2 items-center mt-0.5">
                                    <StatusDot status={t.status} />
                                    <PrioPill priority={t.priority} />
                                </div>
                            </div>
                            <ChevronRight className="w-4 h-4 text-[#C8C4BB]" />
                        </Link>
                    );
                })
            )}
...
function RecentCasesCard({ cases }: { cases: DashCase[] }) {
    ...
    return (
        <Card
            ...
            icon={FileText}
            action={
                <Link href="/cases" className="text-xs font-medium text-[#0D9488] hover:text-[#0F766E]">
                    All cases →
                </Link>
            }
        >
            {cases.length === 0 ? (
                <p className="px-5 py-8 text-sm text-[#8C8880] text-center">No recent cases.</p>
            ) : (
                cases.map(c => (
                    <Link
                        key={c.id}
                        href={`/cases/${c.id}`}
                        className="flex items-center justify-between px-5 py-3.5 border-b border-[#F3F1EC] hover:bg-[#F8F7F5] transition-colors last:border-b-0"
                    >
                        <div className="min-w-0">
                            <p className="text-[13px] font-medium text-[#1C1A17] mb-0.5 truncate">{c.clientName}</p>
                            <div className="flex items-center gap-2">
                                <CaseChip caseNumber={c.caseNumber} />
                                {c.program && <span className="text-[11px] text-[#8C8880]">{c.program}</span>}
                            </div>
                        </div>
                    ...
                    <div key={i}>
                        <div className="flex items-baseline gap-2 mb-1.5">
                            <span
                                ...
                                style={{ fontFamily: 'var(--font-instrument-serif, Georgia, serif)', fontSize: 22, color: i === 0 ? '#0D9488' : '#1C1A17' }}
                            >
                                {day.date.getDate()}
                            </span>
                            <span className={`text-[11px] font-semibold uppercase tracking-[0.15em] ${i === 0 ? 'text-[#0D9488]' : 'text-[#8C8880]'}`}>
                                {day.label}
                            </span>
                        </div>
                        {day.items.length === 0 ? (
                            <p className="text-xs text-[#8C8880] italic">Nothing due</p>
                        ) : (
                            <ul className="flex flex-col gap-1">
                                {day.items.map(t => (
                                    <li key={t.id} className="flex gap-2 text-xs text-[#5C5850]">
            ...
            <div className="p-4 grid grid-cols-2 gap-2.5">
                {actions.map(({ icon: Icon, label, href }) => (
                    <Link
                        key={label}
                        href={href}
                        className="flex items-center gap-2.5 px-3 py-2.5 border border-[#E5E2DB] rounded-lg bg-[#ffffff] text-[13px] text-[#1C1A17] hover:bg-teal-50 hover:border-[#0D9488] transition-colors"
                    >
                        <Icon className="w-3.5 h-3.5 text-[#0D9488] flex-shrink-0" />
                        <span>{label}</span>
                    </Link>
                ))}
            </div>
...
function MessagesCard({ messages }: { messages: DashMessage[] }) {
    return (
        <Card
            ...
            icon={MessageSquare}
            action={
                <Link href="/messages" className="text-xs font-medium text-[#0D9488] hover:text-[#0F766E]">
                    Inbox →
                </Link>
            }
        >
            {messages.length === 0 ? (
                <p className="px-5 py-6 text-sm text-[#8C8880] text-center">No messages.</p>
            ) : (
                messages.slice(0, 4).map(m => (
                    <Link
                        key={m.id}
                        href="/messages"
                        className="flex gap-3 px-4 py-3 border-b border-[#F3F1EC] hover:bg-[#F8F7F5] transition-colors last:border-b-0"
                    >
                        <div
                            className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-semibold flex-shrink-0"
                            style={{ background: m.unread ? '#0D9488' : '#E5E2DB', color: m.unread ? '#fff' : '#5C5850' }}
                        >
                            {m.sender.split(' ').map(w => w[0]).slice(0, 2).join('')}
                        </div>
                        <div className="min-w-0 flex-1">
                            <div className="flex justify-between items-baseline gap-2">
                                <span className={`text-[13px] truncate ${m.unread ? 'font-semibold text-[#1C1A17]' : 'font-medium text-[#1C1A17]'}`}>
                                    {m.sender}
                                </span>
                                <span className="text-[11px] text-[#8C8880] flex-shrink-0">
                                    {format(new Date(m.time), 'MMM d')}
                                </span>
                            </div>
                            <p className="text-xs text-[#5C5850] mt-0.5 truncate">{m.subject || m.content}</p>
                        </div>
                        {m.unread && <span className="w-1.5 h-1.5 rounded-full bg-[#0D9488] self-center flex-shrink-0" />}
                    </Link>
                ))
            )}
        </Card>
...
function CallRequestsCard({ calls }: { calls: CallReq[] }) {
    return (
        <Card title="Call Requests" icon={Phone}>
            {calls.length === 0 ? (
                <p className="px-5 py-6 text-sm text-[#8C8880] text-center">No pending call requests.</p>
            ) : (
                calls.slice(0, 4).map(c => (
                    <div key={c.id} className="flex items-center gap-3 px-4 py-3 border-b border-[#F3F1EC] last:border-b-0">
                        <span className="w-8 h-8 rounded-lg bg-teal-50 text-[#0D9488] flex items-center justify-center flex-shrink-0">
                            <Phone className="w-3.5 h-3.5" />
                        </span>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                                <p className="text-[13px] font-medium text-[#1C1A17] truncate">{c.name}</p>
                                {c.urgent && (
                            ...
                            <p className="text-xs text-[#5C5850] mt-0.5 truncate">{c.reason}</p>
                        </div>
                        <span className="text-[11px] text-[#8C8880] font-mono flex-shrink-0 ml-2">
...
export function UserProfileDashboard() {
    ...
    if (!data) {
        return <div className="p-8 text-center text-[#5C5850]">Failed to load dashboard data.</div>;
    }
                    ...
                    <div className="relative z-10 max-w-[1280px] mx-auto">
                        <div className="flex items-end justify-between gap-8 flex-wrap">
                            ...
                            <div className="min-w-0 flex-1" style={{ minWidth: 0, flex: '1 1 420px' }}>
                                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#0D9488] m-0">
                                    {dateStr}
                                </p>
                                <h1
